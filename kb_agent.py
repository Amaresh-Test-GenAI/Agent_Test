...
def create_agent(kb_id):
    ...
    raw_prompt = (
        ...
        "involved in populating the source to target table. "
        "Given a SQL stored procedure and data model metadata (including source tables, source schema, source data model, and target table), "
        "you must identify SELECT, INSERT, UPDATE, and JOIN operations relevant to those tables. "
        "For each field in the target table, determine the source field(s), the transformation logic, and the transformation type "
        "(Direct Mapping, Expression, Lookup, or No Transformation). "
    ...
    # JSON-safe prompt string
    escaped_prompt = json.dumps(raw_prompt)[1:-1]
    ...
    response = bedrock.create_agent(
        ...
        promptOverrideConfiguration={
            "promptConfigurations": [
                {
                    "basePromptTemplate": json.dumps({"instructions": raw_prompt}),
                    "parserMode": "DEFAULT",
